...
export class CCI extends Component {
				...
				<div className='text-center flex justify-center pt-4'>
					...
					<Link to={`/child/${el.id}`}>
						<img className=""
							...
							alt=''
						/>
						<span className='pt-3 text-center'><strong>name: {el.name}</strong></span>
					</Link>
                    </div>
				</div>
			);
	...
	formatData = () => {
		...
		return <div className='flex container bg-blue-300 pt-10 pb-10 border-2 border-blue-500 flex-col justify-center'>{x}<br /></div>;
	};
				...
				{this.props.role === 'DCPU' && (
					<div className='pl-4 text-center '>
						<button className='text-white font-bold px-6 py-4 rounded outline-none focus:outline-none mr-1 mb-1 bg-gray-800 active:bg-gray-700 uppercase text-sm shadow hover:shadow-lg'>
							<Link
								to={`/cci/${this.props.match.params.id}/edit`}>
								EDIT CCI
							</Link>
						</button>
					</div>
