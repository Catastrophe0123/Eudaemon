...
export class EditCCI extends Component {
	state = { data: null };
	...
	onSubmitHandler = async (keys, values) => {
		...
		try {
			...
			let resp = await axios.put(`/cci/${this.props.match.params.id}`, {
				...obj,
			});
			this.props.history.push(`/cci/${this.props.match.params.id}`);
		} catch (err) {
